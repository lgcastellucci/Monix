...
public class ListOpenWindows : IDisposable
{
    ...
    public List<Window> Search()
    {
        EnumDesktopWindows(IntPtr.Zero, new EnumWindowsProc(EnumWindowsList), IntPtr.Zero);

        var windowsListReturn = new List<Window>();
        foreach (var window in WindowsList)
        {
            if ((window.executablePath = GetExecutablePath(window.hWnd, window.pid)) != "")
            {
                windowsListReturn.Add(window);
            }
        }
        return windowsListReturn;
    }
